...
from flask_hypergen.tags import button, div, h2, input_, label, p
...
def summary_template(
    ...
    *,
    ...
    summary_text = (
        f'{name} is {age} years old and subscribed={subscribed}'
        if submitted
        else 'Submit the form to see a summary.'
    )
    p(summary_text, id_='summary')
...
def submit(request, name: str, age: int, subscribed: bool) -> None:
    summary_template(name, age, subscribed, submitted=True)
